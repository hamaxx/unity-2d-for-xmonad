#!/bin/bash
# Usage: /bin/bash run-with-xvfb.sh <program>
#
# Creates Xvfb instance and runs program in that instance. Shuts down Xvfb when done.

if [ $# -ne 1 ]; then
    echo "Usage /bin/bash run-with-xvfb.sh <program>"
    exit 1
fi
...
if [ "x$DISPLAY" == "x" ]; then
    ...
    { Xvfb -ac -noreset -screen 0 1024x768x16 :$XID -screen 0 1024x768x16 -nolisten tcp -auth /dev/null >/dev/null 2>&1 & trap "kill -15 $! " 0 HUP INT QUIT TRAP USR1 PIPE TERM ; } || { echo "Gtk+Tests:ERROR: Failed to start Xvfb environment for X11 target tests."; exit 1; }
    DISPLAY=:$XID $1
else
