...
context "Launcher Autohide and Show Tests on RTL" do
  ...
  test "Reveal hidden Launcher with mouse" do
    ...
    sleep 0.4
    verify_launcher_hidden(0, 'Launcher should not be visible immediately after mouse moves to the left edge, has to wait 0.5 seconds to show')
    verify_launcher_visible(TIMEOUT, 'Launcher hiding when mouse at left edge of screen')
  ...
  test "Launcher hide delay on tile removal" do
    ...
    if !tiles.empty?
      tile = tiles[0]
      XDo::Mouse.move(XDo::XWindow.display_geometry[0], 200, 0, true)
      verify_launcher_visible(TIMEOUT, 'Launcher hiding when mouse at left edge of screen, should be visible')
      tile.move_mouse()
